...
Private Sub TestReturnMath(ws As Worksheet)
    ...
    AssertNear ActiveReturn(ws.Range("B1:B4"), ws.Range("A5:A8"), 12), _
               ActiveReturn(ws.Range("B1:B4"), ws.Range("A5:A8"), 12), 0.000000001, "ActiveReturn self check"
End Sub

Private Sub TestModifiedDietz(ws As Worksheet)
    ws.Range("C1").Value = 10
    ws.Range("D1").Value = DateSerial(2026, 1, 16)
    AssertNear ModifiedDietzReturn(100, 115, ws.Range("C1"), ws.Range("D1"), _
...
Private Sub TestTimeWeightedReturn(ws As Worksheet)
    ws.Range("E1:E3").Value = Application.Transpose(Array(100, 110, 121))
    ws.Range("F1:F3").Value = Application.Transpose(Array(DateSerial(2026, 1, 1), DateSerial(2026, 1, 2), DateSerial(2026, 1, 3)))
    ws.Range("G1:G2").Value = Application.Transpose(Array(0, 0))
    AssertNear TimeWeightedReturn(ws.Range("E1:E3"), ws.Range("F1:F3"), ws.Range("G1:G2")), _
...
Private Sub TestBrinsonReconciliation(ws As Worksheet)
    ...
    ws.Range("H1:H2").Value = Application.Transpose(Array(0.6, 0.4))
    ws.Range("I1:I2").Value = Application.Transpose(Array(0.1, 0.02))
    ws.Range("J1:J2").Value = Application.Transpose(Array(0.5, 0.5))
    ws.Range("K1:K2").Value = Application.Transpose(Array(0.08, 0.03))

    result = BrinsonFachlerAttribution(ws.Range("H1:H2"), ws.Range("I1:I2"), ws.Range("J1:J2"), ws.Range("K1:K2"))
...
Private Sub TestCompositeReturn(ws As Worksheet)
    ws.Range("L1:L2").Value = Application.Transpose(Array(0.1, 0))
    ws.Range("M1:M2").Value = Application.Transpose(Array(100, 300))
    AssertNear CompositeReturn(ws.Range("L1:L2"), ws.Range("M1:M2")), 0.025, 0.000000001, "CompositeReturn"
End Sub

Private Sub TestCurrencyDecomposition(ws As Worksheet)
    Dim result As Variant
    ws.Range("N1").Value = 0.1
    ws.Range("O1").Value = 0.05
    result = CurrencyReturnDecomposition(ws.Range("N1"), ws.Range("O1"))
...
Private Sub TestFixedIncomeContribution(ws As Worksheet)
    ...
    ws.Range("P1").Value = 1
    ws.Range("Q1").Value = 5
    ws.Range("R1").Value = 0.001
    result = DurationContribution(ws.Range("P1"), ws.Range("Q1"), ws.Range("R1"))
    AssertNear result(1, 1), -0.005, 0.000000001, "DurationContribution"
End Sub

Private Sub TestDrawdown(ws As Worksheet)
    ws.Range("S1:S3").Value = Application.Transpose(Array(0.1, -0.2, 0.05))
    AssertNear MaxDrawdown(ws.Range("S1:S3")), -0.2, 0.000000001, "MaxDrawdown"
    Debug.Assert LongestDrawdown(ws.Range("S1:S3")) = 2
End Sub
